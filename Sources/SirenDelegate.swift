...
@objc public protocol SirenDelegate: NSObjectProtocol {
    ...
    @objc optional func sirenDidShowUpdateDialog(alertType: Siren.AlertType)

    /// User did click on button that launched App Store.app.
    @objc optional func sirenUserDidLaunchAppStore()

    /// User did click on button that skips version update.
    @objc optional func sirenUserDidSkipVersion()

    /// User did click on button that cancels update dialog.
    @objc optional func sirenUserDidCancel()

    /// Siren failed to perform version check (may return system-level error).
    @objc optional func sirenDidFailVersionCheck(error: Error)

    /// Siren performed version check and did not display alert.
    @objc optional func sirenDidDetectNewVersionWithoutAlert(message: String)

    /// Siren performed version check and latest version is installed.
    @objc optional func sirenLatestVersionInstalled()
}
